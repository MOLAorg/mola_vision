...
- [x] **P0.1** Set up proper CMake for `mola_libvision`: find Eigen3, MRPT (v3), TBB, optional GTSAM. **No OpenCV, no Ceres**.
- [x] **P0.2** Implement `sobelGradients()` + `asEigenMap()` zero-copy view: separable 3×3 Sobel using `ptrLine<uint8_t>()` + `getRowStride()`. No `getAsMatrix()` copy needed — `asEigenMap()` wraps `CImage` buffer via `Eigen::Map` with `OuterStride`. Unit tests: step-edge gradients, border zeroing.
- [x] **P0.3** Implement `gaussianBlur()`: separable Gaussian via two 1D convolution passes on `CImage`. Unit tests: delta-function blur shape, flat-image identity.
- [x] **P0.4** Implement `goodFeaturesToTrack()`: structure tensor from Sobel gradients → min-eigenvalue (Shi-Tomasi) or Harris score → NMS → sorted top-N. Unit tests: corners on synthetic checkerboard, max_corners limit, min_distance enforcement.
- [x] **P0.5** Implement `GridFeatureDistributor`: divide image into cells, detect per cell, enforce global min_distance vs. existing tracked features. Unit tests: spatial distribution, avoidance of existing points.
- [x] **P0.6** Implement `calcOpticalFlowPyrLK()`: pyramidal Lucas-Kanade using `CImagePyramid`. Core: 2×2 Hessian solve per level, bilinear sampling, coarse-to-fine propagation. Both CImage and CImagePyramid overloads. Unit tests: known-displacement tracking, graceful handling of blank images.
- [x] **P0.7** Implement `fundamentalMatrixFilter()`: Hartley-normalized 8-point algorithm + RANSAC (Sampson distance), Eigen JacobiSVD, rank-2 enforcement. Unit tests: synthetic inlier/outlier separation.
- [x] **P0.8** Implement batch `undistortPoints()`: wraps MRPT's `undistort_point()` for feature vectors. Unit test: no-distortion camera → normalized coords.
- [x] **P0.9** Implement `triangulatePoints()` (DLT batch) + `triangulateSinglePoint()`: SVD of 4×4 design matrix, chirality check. Unit tests: known 3D points recovery < 1mm error, behind-camera rejection.
- [x] **P0.10** Port `MapPoint` class: position (MRPT + Eigen accessors), observation list (keyframe weak_ptr + feature index), 3×3 covariance, pixel-space covariance projection. Thread-safe. Unit tests: add/remove obs, covariance projection formula.
- [x] **P0.11** Implement `Keyframe` struct + `Feature` struct: wraps `mrpt::img::CImage` (gray+right+depth), `TCamera`, pose, velocity, IMU data, features, covisibility back-links.
- [x] **P0.12** Implement `LevenbergMarquardt` solver (header-only template): Eigen LDLT, Huber kernel, adaptive λ (Nielsen update), convergence checks. Euclidean and manifold retraction via callable. Unit tests: linear LS (exact solution), Rosenbrock (known minimum), dynamic-size variant, `huberWeight()` formula.
